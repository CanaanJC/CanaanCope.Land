...
//
// Walks `library.depth` levels of folders under public/libraries/{library.path}
// (libraries are physically stored under public/libraries/ — see
// lib/staticFile.js's resolveFsPath for the matching transparent rewrite used
// when serving their files directly). Every folder at every level gets its
// {num, title} parsed (used by title-mode/non-date libraries for nav). At the
// final depth level, a folder is a leaf only if it has a config.json; leaves
// with `block: true` are skipped entirely (invisible to manifest + routes,
// same as before).
