...
function buildLibraryEmbedHtml(library, slugParts, config, origin, options = {}) {
    ...
    const basePath     = `${origin}/${library.path}/${encodedParts.join("/")}`;
    const url          = single ? `${basePath}?single` : basePath;
    ...
    const imageTag = buildImageTag(
        mediaDir,
        (file) => `${basePath}/media/${encodeURIComponent(file)}`
    );

    const head = buildHeadMeta({ title, description, url, imageTag });

    const isolated = config.block === true || config.private === true || single;
...
${head}
  ...
  <body>
    ...
    <script>
      ...
      window.__LIBRARY_SINGLE__ = ${single ? "true" : "false"};
    </script>
