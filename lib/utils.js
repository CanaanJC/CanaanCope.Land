...
//
// Inserts a snippet immediately before the closing </head> tag of an HTML
// document string. Used to inject the Twemoji CDN <script> tag (and
// nothing else needs to change per-file — no .html file on disk ever
// carries this tag itself). If the document has no </head> at all, the
// html is returned completely untouched — never throws, never corrupts
// the page.
...
// Convenience wrapper specifically for the Twemoji script tag, since this
// is injected into every HTML response sitewide (see lib/staticFile.js,
// lib/routes.js, lib/embed.js).
