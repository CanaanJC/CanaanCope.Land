...
// `tag` behavior:
//   - If explicitly passed (a non-null string), that exact string is used.
//   - Otherwise, if `force` is true (a manual/terminal-triggered backup with
//     no extra text given), falls back to "manual terminal backup".
//   - Otherwise (a normal scheduled/cron backup), falls back to
//     "<interval> backup" (e.g. "monthly backup", "daily backup").
...
function startTerminalCommands() {
    ...
    rl.on("line", (line) => {
        ...
        // First word (case-insensitively) must be "backup" to trigger
        // anything. Everything after the first space, taken verbatim
        // (original casing preserved), becomes the tag — e.g.
        // "backup this is a test" → tag "this is a test". Bare "backup"
        // with nothing after it falls back to "manual terminal backup"
        // (handled by performBackup's default when tag is null).
