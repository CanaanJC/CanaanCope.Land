...
async function sendBackupWebhook({ uuid, sizeBytes, timestamp, nextBackupAt }) {
...
async function performBackup({ force = false } = {}) {
    ...
    logB(`starting backup for "${siteName}"${force ? " (manual trigger)" : ` (interval: ${cfg.interval})`}`);
    ...
    await sendBackupWebhook({ uuid, sizeBytes, timestamp: now, nextBackupAt });
}
