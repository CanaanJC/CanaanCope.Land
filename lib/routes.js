...
const { getLibraryManifest, compareFolderNames } = require("./manifest");
...
function sortManifestForLibrary(library, manifest) {
    if (!Array.isArray(manifest)) return [];
    if (!library.useDates) return manifest;

    return [...manifest].sort((a, b) => {
        const da = parseFlexibleDate(getEndDate(a.date));
        const db = parseFlexibleDate(getEndDate(b.date));
        if (!da && !db) return 0;
        if (!da) return 1;
        if (!db) return -1;
        return db - da;
...
function firstEntryUnderFolder(manifest, folderParts) {
    ...
    return match ? match.slugPath : null;
}

function resolveFolderTargetSlug(library, folderParts) {
    const manifest = sortManifestForLibrary(library, getLibraryManifest(library));
    ...
    if (preceding) return preceding.slugPath;
    ...
    if (parts.length > 0) {
        ...
        if (library) {
            ...
            if (
                ...
                rest.length > 0 &&
                rest.length < library.depth &&
                rest.every(isSafeSegment)
            ) {
