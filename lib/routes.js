...
const { getLibraries, getLibraryByPath, getSiteInfo, getTheme, getWebhooks } = require("./siteConfig");
...
//
// The homepage's embed is never hardcoded. At request time we:
//   1. Read public/index.html's literal <title>…</title> content.
//   2. Read "slogan" from config/master.json (via getSiteInfo()).
//   3. Always point og:image / twitter:image at /media/logo.png.
//
// If title/slogan are missing, their tags are simply left blank — nothing
// is ever backfilled from some other field. If the <title> tag can't be
// found at all (or </head> can't be found to inject into), NO embed tags
// are added whatsoever — the page is served exactly as written on disk,
// with zero embed rather than a partial/guessed one.
...
function serveHomepageWithEmbed(req, res, stdHeaders) {
    ...
    // If there's no <title> tag, or no </head> to inject into, html is left
    // completely untouched — served as-is, with no embed tags at all.
...
function handleRoutes(req, res, safePath, stdHeaders) {
    ...
    if (safePath === "/config/theme.json") {
        const { slogan } = getSiteInfo();
        const theme   = getTheme();
        const favicon = getFaviconUrl();
        sendJson(res, stdHeaders, JSON.stringify({ favicon, slogan, theme }), "public, max-age=60");
        return true;
    }
