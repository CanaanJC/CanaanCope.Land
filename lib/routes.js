...
function handleRoutes(req, res, safePath, stdHeaders) {
    ...
    // entry's layout, per the aboutme consolidation. NOTE: public/aboutme/
    // is NOT a library and is never moved under public/libraries/.)
    ...
    // NOTE: library.path is still the bare config path (e.g. "template") —
    // libraries are physically stored under public/libraries/<path> on disk
    // (see lib/staticFile.js's resolveFsPath for the transparent rewrite
    // that also applies to manifest.json/content.md/media served directly
    // by the static file handler). Only the two on-disk lookups below
    // (media-listing folder + this entry's config.json) need the explicit
    // "libraries" segment — getLibraryManifest already handles it internally.
