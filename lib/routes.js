...
const { getLibraries, getLibraryByPath, getSiteInfo, getTheme, getFontsConfig, getWebhooks, CONFIG_DIR } = require("./siteConfig");
...
// theme.bottomText.font are each either "" (off) or a path RELATIVE TO
// public/ (same convention as media/logo.png, theme.topbar.icon, etc. —
// normally "fonts/MyFont.woff2", but any public/-relative path works)
// pointing at a .otf/.ttf/.woff/.woff2 file.
//
// config/master.json's top-level "fonts" section provides two more paths,
// resolved the exact same way:
//   - fonts.fallback — a site-wide "house font" used for any of the 3
//     fields above that doesn't have its own font set (or whose own font
//     fails to resolve). If this is also empty/invalid, that field falls
//     through to pure browser default, same as always.
//   - fonts.flag     — the Windows flag-emoji fix font (see below). No
//     longer hardcoded to a fixed filename — configurable here instead.
//
...
// The 3 user-configurable font fields. Each resolves its own font first;
// if that's off/invalid, falls through to fonts.fallback (shared site-wide
// "house font", also resolved the same way); if that's also off/invalid,
// resolves to null (pure browser default) — exactly as before.
function resolveThemeFonts(theme, fontsCfg) {
    const fallback = () => resolveFontField("site-fallback-font", fontsCfg?.fallback);

    return {
        page:       resolveFontField("custom-page-font",       theme?.page?.font)          || fallback(),
        slogan:     resolveFontField("custom-slogan-font",      theme?.topbar?.slogan?.font) || fallback(),
        bottomText: resolveFontField("custom-bottom-text-font", theme?.bottomText?.font)     || fallback(),
    };
}
...
// glyphs. config/master.json's fonts.flag points at whichever font file
// you want to use to patch this (must actually contain color flag
// artwork to look right — see project notes). Resolved the same way as
// every other font field above; if it doesn't resolve, this is simply
// null and Windows renders flags exactly as it does today — no error, no
// broken page either way.
function resolveFlagFont(fontsCfg) {
    return resolveFontField("twemoji-flags", fontsCfg?.flag);
}
...
function handleRoutes(req, res, safePath, stdHeaders) {
    ...
    // "fonts" carries the 3 user-configurable resolved custom fonts (each
    // already falling back to fonts.fallback if its own is off/invalid, or
    // null if neither resolves). "flagFont" carries the configurable
    // Windows flag-emoji fix font (or null if fonts.flag doesn't resolve).
    if (safePath === "/config/theme.json") {
        const { slogan, bottomText } = getSiteInfo();
        const theme    = getTheme();
        const fontsCfg = getFontsConfig();
        const favicon  = getFaviconUrl();
        const fonts    = resolveThemeFonts(theme, fontsCfg);
        const flagFont = resolveFlagFont(fontsCfg);
