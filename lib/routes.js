...
//      for older configs.
//   2. Overwrite the literal <title>…</title> tag in the HTML with that
//      resolved title, so the browser tab / bookmark name always matches
//      config/master.json's "siteName" live — no restart, no rebuilding
//      index.html by hand.
...
// can't be found to inject into, NO embed tags are added whatsoever and the
// <title> tag is left completely untouched — the page is served exactly as
// written on disk, with zero embed rather than a partial/guessed one.
...
function serveHomepageWithEmbed(req, res, stdHeaders) {
    ...
    if (title && /<\/head>/i.test(html)) {
        // Overwrite the literal <title> tag so the browser tab/bookmark
        // name always reflects config/master.json's siteName live.
    ...
    // If there's no resolvable title at all, or no </head> to inject into,
    // html is left completely untouched — served as-is, with no embed
    // tags and no <title> rewrite.
