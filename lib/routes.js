...
//
// config/master.json's top-level "fonts" section provides one more path,
// resolved the exact same way:
//   - fonts.master — a site-wide "house font" used for any of the 3
...
//     through to the CSS-baked system-ui stack (see base.css/topbar.css/
//     bottom-page.css), same as always.
//
...
//
// NOTE: the previous "fonts.flag" Windows flag-emoji @font-face fix has
// been removed entirely. Flag emoji (and all other emoji) are now handled
// client-side by Twemoji (see public/js/twemoji-init.js), whose <script>
// tag is injected server-side into every HTML response — see
// lib/utils.js: injectTwemoji, and its call sites in this file /
// lib/staticFile.js / lib/embed.js.
...
// if that's off/invalid, falls through to fonts.master (shared site-wide
// "house font", also resolved the same way); if that's also off/invalid,
// resolves to null (client-side CSS system-ui stack takes over) — exactly
// as before.
function resolveThemeFonts(theme, fontsCfg) {
    const masterFallback = () => resolveFontField("site-master-font", fontsCfg?.master);

    return {
        page:       resolveFontField("custom-page-font",       theme?.page?.font)          || masterFallback(),
        slogan:     resolveFontField("custom-slogan-font",      theme?.topbar?.slogan?.font) || masterFallback(),
        bottomText: resolveFontField("custom-bottom-text-font", theme?.bottomText?.font)     || masterFallback(),
    };
}
...
function handleRoutes(req, res, safePath, stdHeaders) {
    ...
    // already falling back to fonts.master if its own is off/invalid, or
    // null if neither resolves). The previous "flagFont" field (Windows
    // flag-emoji fix) has been removed — emoji rendering is now handled
    // entirely client-side by Twemoji, injected server-side into every
    // HTML response.
