...
const { fileExists, dirExists, listSupportedMedia } = require("./fsCache");
const { getLibraryManifest, isBlogDir } = require("./manifest");
...
function wantsSingle(req) {
    ...
    if (!params.has("single")) return false;
    const value = (params.get("single") || "").toLowerCase();
    return value !== "0" && value !== "false";
}

function rawSearch(req) {
    const raw = req.url || "";
    const i = raw.indexOf("?");
    if (i === -1) return "";
    const q = raw.slice(i);
...
function sendRedirect(req, res, stdHeaders, location) {
...
function libraryDir(library, parts = []) {
    return path.join(PUBLIC_DIR, "libraries", library.path, ...parts);
}

function resolveFolderTargetSlug(library, folderParts) {
...
function resolveLocalFontFile(familyName, relPath) {
    ...
    return { kind: "local", family: familyName, url: urlPath, format };
}

function resolveFontField(familyName, value) {
    if (typeof value !== "string" || value.trim() === "") return null;
    const raw = value.trim();

    const descriptor = parseGoogleFont(raw);
    if (descriptor) {
        const stylesheet = googleFontStylesheetUrl(descriptor);
        ...
        return {
            ...
            weight:            googleFontWeight(descriptor),
            style:             googleFontStyle(descriptor),
            variationSettings: googleFontVariationSettings(descriptor),
        };
    }

    if (/^[a-z][a-z0-9+.-]*:/i.test(raw)) return null;

    return resolveLocalFontFile(familyName, raw);
}
...
function buildThemePayload(theme, themeDefaults) {
    ...
    const dropdownSection           = t.dropdown || {};
    const dropdownDefaultsSection   = td.dropdown || {};
    const dropdownFolderSection     = dropdownSection.folder || {};
    const dropdownFolderDefaults    = dropdownDefaultsSection.folder || {};
    const dropdownBlogSection       = dropdownSection.blog || {};
    const dropdownBlogDefaults      = dropdownDefaultsSection.blog || {};
    ...
    const colorTheme = {
        ...
        dropdown: {
            dividerColor: strOrNull(dropdownSection.dividerColor) || strOrNull(dropdownDefaultsSection.dividerColor) || null,
            dividerWeight: typeof dropdownSection.dividerWeight === "number"
                ? dropdownSection.dividerWeight
                : (typeof dropdownDefaultsSection.dividerWeight === "number" ? dropdownDefaultsSection.dividerWeight : 1),
            folder: {
                fontSize:        resolveWithMasterOverride(dropdownFolderSection.fontSize,  dropdownFolderDefaults.fontSize,  masterResolved.fontSize),
                textColor:       resolveWithMasterOverride(dropdownFolderSection.textColor, dropdownFolderDefaults.textColor, masterResolved.textColor),
                mobileTextColor: strOrNull(dropdownFolderSection.mobileTextColor) || strOrNull(dropdownFolderDefaults.mobileTextColor) || null,
            },
            blog: {
                fontSize:        resolveWithMasterOverride(dropdownBlogSection.fontSize, dropdownBlogDefaults.fontSize, masterResolved.fontSize),
                textColor:       strOrNull(dropdownBlogSection.textColor) || strOrNull(dropdownBlogDefaults.textColor) || null,
                mobileTextColor: strOrNull(dropdownBlogSection.mobileTextColor) || strOrNull(dropdownBlogDefaults.mobileTextColor) || null,
            },
        },
        ...
        code: {
            inlineBackgroundColor: resolveWithMasterOverride(codeSection.inlineBackgroundColor, codeDefaultsSection.inlineBackgroundColor, masterResolved.backgroundColor),
            ...
            inlineBorderColor:     strOrNull(codeSection.inlineBorderColor),
    ...
    const fonts = {
        ...
        dropdownFolder: resolveFontField("custom-dropdown-folder-font", resolveFontPath(dropdownFolderSection, dropdownFolderDefaults, masterFontPath)),
        dropdownBlog:   resolveFontField("custom-dropdown-blog-font",   resolveFontPath(dropdownBlogSection, dropdownBlogDefaults, masterFontPath)),
    };
...
function handleRoutes(req, res, safePath, stdHeaders) {
    ...
    if (parts.length > 0) {
        ...
        if (library) {
            ...
            const listingIndex = rest.indexOf("media-listing");
            if (listingIndex !== -1) {
                const slugParts = rest.slice(0, listingIndex);
                const folder    = rest[listingIndex + 1];

                if (!folder || !slugParts.every(isSafeSegment) || !isSafeSegment(folder)) {
                    sendJsonBadRequest(res, stdHeaders);
                    return true;
                }

                const folderPath = libraryDir(library, [...slugParts, "media", folder]);
                sendJson(res, stdHeaders, JSON.stringify(listSupportedMedia(folderPath)));
                return true;
            }

            if (rest.length > 0 && rest.every(isSafeSegment)) {
                const targetDir = libraryDir(library, rest);

                if (dirExists(targetDir) && isBlogDir(targetDir)) {
                    const configPath = path.join(targetDir, "config.json");
                    try {
                        const config = JSON.parse(fs.readFileSync(configPath, "utf-8"));
                        const origin = `${req.headers["x-forwarded-proto"] || "http"}://${req.headers.host}`;
                        const single = wantsSingle(req);
                        const html   = buildLibraryEmbedHtml(library, rest, config, origin, { single });
                        res.writeHead(200, {
                            ...stdHeaders,
                            "Content-Type":  "text/html; charset=utf-8",
                            "Cache-Control": "no-store",
                ...
                if ((req.method === "GET" || req.method === "HEAD") && dirExists(targetDir)) {
                    const slugPath = resolveFolderTargetSlug(library, rest);
                    if (slugPath && slugPath.length) {
                        const target = `/${library.path}/${slugPath.map(encodeURIComponent).join("/")}${rawSearch(req)}`;
                        sendRedirect(req, res, stdHeaders, target);
                        return true;
                    }
                    sendRedirect(req, res, stdHeaders, `/${library.path}${rawSearch(req)}`);
                    return true;
                }
            }
        }
