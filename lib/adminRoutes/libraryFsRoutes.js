...
async function handleLibraryFsRoutes(req, res, safePath, method) {
    ...
    //
    // Body: { path, name, depth, useDates, hidden, icon }
    ...
    //   hidden   → true = library exists but isn't listed publicly.
    //              ALWAYS written to libraries.json (defaults to false when
    //              omitted) so the field is present from the moment the
    //              library is created.
    ...
    if (safePath === "/api/library-fs/new-library" && method === "POST") {
        try {
            ...
            // Explicit boolean coercion — anything other than a literal
            // true (missing, null, "false", 0, …) means NOT hidden.
    ...
    //
    // Body: { path, name?, hidden? }
    //
    // Only ever touches libraries.json — never the folder on disk, and
    // never the library's `path`/`id`. Renaming the display name is safe
    // (nothing links to it); renaming the PATH would break every existing
    // URL, bookmark and embed pointing at that library, so it deliberately
    // isn't offered here. Fields are applied only if present in the body,
    // so this same endpoint can toggle `hidden` without touching `name`
    // and vice versa.
    ...
    // Body: { path }
    //
    // Removes BOTH the libraries.json entry and the whole
    // public/libraries/<path>/ folder (every folder, blog, and media file
    // inside it). Irreversible — the frontend is responsible for
    // confirming first.
    //
    // Order matters: libraries.json is rewritten FIRST. If that write
    // fails, nothing is deleted from disk, so the site is never left
    // pointing at a library whose files are gone. The reverse order could
    // leave a dead entry serving 404s.
    if (safePath === "/api/library-fs/delete-library" && method === "POST") {
        try {
            ...
            // Re-resolved through the same guard every other path in this
            // file uses, so a crafted `path` can never escape
            // public/libraries/.
            ...
            if (targetDir && targetDir !== LIBRARIES_ROOT) {
                try {
                    ...
                    // Entry is already gone from libraries.json, so the
                    // library is effectively deleted as far as the site is
                    // concerned — report the leftover folder rather than
