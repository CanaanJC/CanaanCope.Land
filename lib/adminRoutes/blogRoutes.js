...
// to stay inside PUBLIC_DIR. Deliberately only ever allows "content.md" —
// a blog's config.json is edited through the generic /api/file endpoint.
//
// SPECIAL CASE: the reserved "aboutme" path resolves to
// public/aboutme/content.md — that page lives OUTSIDE public/libraries/
// but is edited through the exact same markdown editor. See lib/aboutMe.js.
function resolveSafeBlogPath(urlPath, filename) {
    ...
    if (isAboutMePath(urlPath)) {
        // Self-heal: if the folder/file were removed outside the app after
        // boot, recreate them rather than 404ing the editor.
        ensureAboutMe();
...
// config.json) — purely presence-based, and intentionally does NOT respect
// a leaf's `block` flag (the editor should still be able to open a blocked
// entry to un-block it).
...
async function handleBlogRoutes(req, res, safePath, method) {
    ...
    // The About Me page is deliberately NOT included here — it isn't a
    // library blog, and the Library Explorer injects it on its own, pinned
    // to the bottom of the library list.
