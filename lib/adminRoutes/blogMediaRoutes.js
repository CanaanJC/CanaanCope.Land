...
const { isAboutMePath, ABOUT_ME_MEDIA_DIR, ensureAboutMe } = require("../aboutMe");
...
//
// SPECIAL CASE: the reserved "aboutme" path maps to public/aboutme/media/
// instead of public/libraries/…/media/ — the About Me page lives outside
// the libraries tree but uses this exact same media manager.
function resolveSafeMediaDir(urlPath, subPath) {
    ...
    if (isAboutMePath(urlPath)) {
        ensureAboutMe();
        resolved = path.resolve(ABOUT_ME_MEDIA_DIR, ...subSegments);
    } else {
        const segments = urlPath.split("/").filter(Boolean);
        if (segments.length === 0 || !segments.every(isSafeSegment)) return null;
        resolved = path.resolve(PUBLIC_DIR, "libraries", ...segments, "media", ...subSegments);
    }
...
async function handleBlogMediaRoutes(req, res, safePath, method) {
    ...
    // Body: { path, name, type: "file"|"folder", fromSub, toSub }.
