...
// ── Date mode is only supported at depth 1 ────────────────────────────────
//
// Date-mode ("By Month") navigation flattens an entire library into a
// two-level Year → Month tree, keyed off each entry's own `date` field.
// That model has no way to represent the intermediate folder levels a
// deeper library has — buildTopbarDateNav() in public/js/library.js (and
// its mobile twin in mobile.js) simply drops every segment except the
// leaf, so a depth-2+ library in date mode renders a nav tree that can't
// actually address most of its own structure, and any entry whose date is
// missing or unparseable vanishes from the nav entirely.
//
// So: ANY library with depth > 1 is treated as `useDates: false`, full
// stop — it always uses title mode (the recursive N-level "Contents" tree
// built from folder names, sorted by their numeric prefix). This is a
// read-time normalization only; the value in libraries.json is left
// untouched on disk, so flipping a library back to depth 1 restores
// whatever useDates it was originally configured with.
//
// Deliberately silent — this is normal, expected behavior for any deep
// library, not a warning-worthy condition, and it was previously logged
// on every single read of the config.
function normalizeUseDates(lib) {
    if (lib.depth > 1) return false;
    return lib.useDates === true;
}
...
function getLibraries() {
    ...
    return list
        ...
        .map(lib => ({
            ...
            useDates: normalizeUseDates(lib),
        }));
}
