...
const HARD_FALLBACK_THEME_DEFAULTS = {
    ...
    dropdown: {
        ...
        folder: {
            ...
            fontSize: 16,
            textColor: "#eaeaea",
            mobileTextColor: "#eaeaea",
        },
        blog: {
            font: "",
            fontSize: 14,
            textColor: "#545454",
            mobileTextColor: "#545454",
        },
    },
    bottomText: {
        font: "",
    ...
    code: {
        inlineBackgroundColor: "#252525",
        inlineBorderColor: "#333333",
        textColor: "#eaeaea",
...
const HARD_FALLBACK_COMBINED_DEFAULTS = {
    defaults: HARD_FALLBACK_MASTER_DEFAULTS,
    themeDefaults: HARD_FALLBACK_THEME_DEFAULTS,
};
...
function getCombinedDefaults() {
    ...
    if (existing && typeof existing === "object" && existing.defaults && existing.themeDefaults) {
        return existing;
    }
    writeJsonSafe(DEFAULTS_PATH, HARD_FALLBACK_COMBINED_DEFAULTS);
    return HARD_FALLBACK_COMBINED_DEFAULTS;
}

function getMasterDefaults() {
    return getCombinedDefaults().defaults;
}

function getThemeDefaults() {
    return getCombinedDefaults().themeDefaults;
}
...
function getLibraries() {
    ...
    return list
        .filter(lib => lib && typeof lib.path === "string" && lib.path.length > 0)
        .map(lib => ({
            ...lib,
            private: lib.private === true || lib.hidden === true,
