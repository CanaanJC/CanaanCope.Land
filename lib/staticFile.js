...
// Serves the custom 404 page (public/404/404.html). Not a redirect — the
// browser's address bar keeps whatever URL was actually requested; only the
// response body/status change. Falls back to plain text if the 404 page
// itself is somehow missing, so a broken/missing 404.html can never itself
...
function serveStaticFile(req, res, safePath, stdHeaders) {
    ...
    const cacheControl  = servingOriginalFallback ? FALLBACK_CACHE : getCacheControl(ext);
