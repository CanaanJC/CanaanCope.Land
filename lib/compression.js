...
function isNoCompressName(name) {
    return name.startsWith(NO_COMPRESS_PREFIX);
}

function isNoCompress(fsPath) {
    return isNoCompressName(path.basename(fsPath));
}
...
function getVariantInfo(fsPath) {
    ...
    if (isNoCompressName(path.basename(fsPath))) return null;
...
async function buildVariant(originPath, info) {
    ...
    if (isNoCompress(originPath)) {
        logC(`skip ${rel} — "${NO_COMPRESS_PREFIX}" prefix marks this file as no-compress`);
        return;
    }
...
function scheduleVariantBuild(originPath, info) {
    if (!info) return;
    if (isNoCompress(originPath)) return;
...
function cleanCmpsdDir(cmpsdDir) {
    ...
    for (const name of files) {
        ...
        if (isNoCompressName(name)) {
            try {
                fs.unlinkSync(variantPath);
                invalidateStat(variantPath);
                for (const candidate of candidates) deleteEntry(stateKey(candidate));
                removed++;
                logC(`removed variant ${relPub(variantPath)} ("${NO_COMPRESS_PREFIX}" prefix marks origin as no-compress)`);
            } catch (e) {
                logC(`error: failed to remove no-compress variant ${relPub(variantPath)}: ${e.message}`);
            }
            continue;
        }
...
function collectOrigins(dir, out) {
    ...
    for (const entry of entries) {
        ...
        if (isNoCompressName(entry.name)) continue;
...
async function runHashScan(rootDir = PUBLIC_DIR) {
    ...
    for (const key of Object.keys(state.entries)) {
        ...
        if (isNoCompressName(path.basename(absolute))) {
            deleteEntry(key);
            pruned++;
            continue;
        }
